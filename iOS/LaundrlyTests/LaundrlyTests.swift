import XCTest
@testable import Laundrly

@MainActor
final class LaundrlyTests: XCTestCase {
    var store: LaundrlyStore!
    ...
    func testFreeLimitBlocksAddition() {
        store.isPro = false
        for i in 0..<(LaundrlyStore.freeLimit + 5) {
            store.add(title: "Filler \(i)", amount: 1.0)
        }
        XCTAssertEqual(store.entries.count, LaundrlyStore.freeLimit)
    }
    ...
    func testCanAddMoreWhenPro() {
        ...
        XCTAssertGreaterThan(store.entries.count, LaundrlyStore.freeLimit)
    }
    ...
    func testFreeLimitAboveSeedCount() {
        XCTAssertGreaterThan(LaundrlyStore.freeLimit, 3)
    }
}
